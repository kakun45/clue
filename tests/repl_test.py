...
class ReplTests(unittest.TestCase):
    ...
    def test_resolve_card(self):
        ...
        self.assertEqual([clue.MUSTARD], repl.resolve_card(clue.MASTER_DETECTIVE, "mu"))
        self.assertEqual(
            sorted([clue.DINING, clue.DRAWING]),
            sorted(repl.resolve_card(clue.MASTER_DETECTIVE, "d")),
        )
    ...
    def test_update_entry(self):
        ...
        ClueRepl.update_entry(
            clue.MASTER_DETECTIVE, log_entry, asker="Dave", cards=[], answers=[]
        )
        self.assertEqual("Dave", log_entry.asker)
        ClueRepl.update_entry(
            clue.MASTER_DETECTIVE,
            log_entry,
            asker="Dave",
            cards=[clue.KNIFE],
            answers=[],
    ...
    def test_parse_set_line(self):
        ...
        self.assertEqual(
            ([clue.PLUM], "dave", clue.HAS_CARD),
            ClueRepl.parse_set_line(clue.MASTER_DETECTIVE, line),
        )
        line2 = "set KNI dave=N"
        self.assertEqual(
            ([clue.KNIFE], "dave", clue.DOESNT_HAVE_CARD),
            ClueRepl.parse_set_line(clue.MASTER_DETECTIVE, line2),
        )
